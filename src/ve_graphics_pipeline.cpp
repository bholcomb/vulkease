...
VEGraphicsPipelineDesc veDefaultGraphicsPipelineDesc(void)
{
   ...
   desc.frontStencil = {VK_STENCIL_OP_KEEP, VK_STENCIL_OP_KEEP, VK_STENCIL_OP_KEEP, VK_COMPARE_OP_ALWAYS, 0xFF, 0xFF, 0};
                                   ...
                                   VEGraphicsPipeline **outPipeline)
{
   ...
   {
      uint32_t count = desc->vertexBindingCount < VE_MAX_VERTEX_BINDINGS ? 
                       desc->vertexBindingCount : VE_MAX_VERTEX_BINDINGS;
   ...
   {
      uint32_t count = desc->vertexAttributeCount < VE_MAX_VERTEX_ATTRIBUTES ?
                       desc->vertexAttributeCount : VE_MAX_VERTEX_ATTRIBUTES;
   ...
   {
      uint32_t count = desc->blendAttachmentCount < VE_MAX_COLOR_ATTACHMENTS ?
                       desc->blendAttachmentCount : VE_MAX_COLOR_ATTACHMENTS;
...
static VEResult createPipelineWithBlend(VEDevice *device, VEShader *vertexShader, VEShader *fragmentShader,
                                        ...
                                        bool blendEnable, VkBlendFactor srcColor, VkBlendFactor dstColor,
...
VEResult veCreateOpaquePipeline(VEDevice *device, VEShader *vertexShader, VEShader *fragmentShader,
                                 ...
                                 const char *debugName, VEGraphicsPipeline **outPipeline)
{
   return createPipelineWithBlend(device, vertexShader, fragmentShader,
                                   bindings, bindingCount, attrs, attrCount,
                                   false, VK_BLEND_FACTOR_ONE, VK_BLEND_FACTOR_ZERO,
                                   true, true, debugName, outPipeline);
}
