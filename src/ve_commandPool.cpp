...
extern "C" bool veInitCommandPool(VEDeviceInternal* device, uint32_t queueFamily, VECommandPool* pool)
{
    ...
    pool->commandBuffers = static_cast<VECommandBufferInternal*>(
        calloc(VE_MAX_COMMAND_BUFFERS, sizeof(VECommandBufferInternal)));
    pool->commandBufferInUse = static_cast<bool*>(
...
extern "C" VEResult veAllocateCommandBuffer(VEDeviceInternal* device, VECommandPool* pool, VECommandBufferInternal** outCmd) {
    ...
    for (uint32_t i = 0; i < VE_MAX_COMMAND_BUFFERS; i++) {
        if (!pool->commandBufferInUse[i]) {
            if (!pool->commandBuffers[i].commandBuffer) {
                ...
                VkResult result = vkAllocateCommandBuffers(device->device, &allocInfo, &pool->commandBuffers[i].commandBuffer);
                if (result != VK_SUCCESS) {
